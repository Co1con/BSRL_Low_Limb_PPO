...
All the environments are registered in the `BSRL_Low_Limb_PPO` extension. They start
...
with contextlib.suppress(ImportError):
    import BSRL_Low_Limb_PPO.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~BSRL_Low_Limb_PPO.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from BSRL_Low_Limb_PPO.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `BSRL_Low_Limb_PPO` extension."""
    ...
    if args_cli.show_presets:
        from BSRL_Low_Limb_PPO.utils.preset_cli import enumerate_task_presets
